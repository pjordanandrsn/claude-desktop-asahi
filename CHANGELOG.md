...
- Cowork Linux patches apply again on Claude Desktop 1.13576+ (the build's "Verify cowork patches in shipped asar" step was failing with 9/11 markers missing). Upstream re-architected the cowork/VM subsystem ("yukonSilver"): the platform gate moved from a `darwin`/`win32` check into `startVM`'s `yukonSilver.status` feature-flag check, the vmClient module load moved behind the isMsix detector, and `sharedCwdPath`/`mountConda` were removed. Patches 1, 2 and the daemon auto-launch anchor were re-derived against the new bundle, the smol-bin idempotency guard was fixed (it false-matched upstream's own log), and the obsolete `sharedCwdPath` threading (Patch 12) was retired in favor of the daemon's mountMap fallback. The marker set also gained a fingerprint for the Linux smol-bin copy patch, which was load-bearing but previously unverified. ([#1](https://github.com/pjordanandrsn/claude-desktop-asahi/issues/1))
